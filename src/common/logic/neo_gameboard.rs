use bevy::{prelude::*};
use bevy_fast_tilemap::{MapReadyEvent, Map};
use noise::{Perlin, utils::{PlaneMapBuilder, NoiseMapBuilder, NoiseMap}, Fbm};
use rand::Rng;

use crate::common::config::Config;

use super::{TileFeature, TileGeography, TeamColour};
...
pub fn spawn_gameboard(
    ...
    for ev in evs.iter() {
        ...
        let scale = u32::max(gameboard_config.width, gameboard_config.height) as f64 * (1f64 / 32f64);
        ...
        let heightmap = new_perlin_noise(scale, heightmap_seed, gameboard_config.width, gameboard_config.height);
        let inlandness = new_perlin_noise(scale, inlandness_seed, gameboard_config.width, gameboard_config.height);
        let climate = new_perlin_noise(scale, climate_seed, gameboard_config.width, gameboard_config.height);

        for x in 0..gameboard_config.width {
            gameboard.tiles.push(Vec::with_capacity(gameboard_config.height as usize));
            for y in 0..gameboard_config.height {
                ...
                gameboard.tiles.get_mut(x as usize).unwrap().push(Tile { contents: tile, feature: None, visible_for: Vec::new() });
            }
        }
